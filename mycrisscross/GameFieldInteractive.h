...
class GameFieldInteractive
{
	private:
		...
		//int MoveCounter;  счетчик числа ходов, увеличиваетс€  на 1 при каждом ходе, используетс€ дл€ определени€ ничьей

		//int CombinationsField[8][2];
		//массив на 8 строк и 2 столбца, 1 столбец - число крестиков, 2 столбец - число ноликов, 1 строка - главна€ диагональ, 2 строка - побочна€ диагональ
		//3-5 строки - 1-3 строки пол€(снизу вверх), 6-8 строки - 1-3 столбцы пол€(слева направо)

		struct fieldid //структура преобразующа€ номер пол€, вводимый польщователем в номер пол€ в массиве gamefield
		{
			int field1;
			int field2;
		...
		bool CheckMovePossibility(int fieldnumber)//метод определ€ющий возможность походить исход€ из наличи€ крестика/нолика в провер€емом поле
		{
